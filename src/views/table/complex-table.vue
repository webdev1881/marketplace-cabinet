  ...
  <div class="app-container">
    <div style="margin-bottom: 20px" class="filter-container">
      <el-input
        ...
        style="width: 370px; margin: 0 10px 10px 0"
        class="filter-item"
    ...
    >
      ...
      <el-table-column :label="'name' | localize" width="300px">
        <template slot-scope="{ row }">
          <div class="pointer" @click="handleUpdate(row)">
            <span class="link-type">{{ row.name_uk }}</span>
          </div>
        </template>
      </el-table-column>
      ...
      <el-table-column label="Товарна група" width="200px" align="center">
        <template slot-scope="{ row }">
          <span class="link-type custom_group" @click="handleUpdate(row)">{{
            row.category.name
          }}</span>
        </template>
      </el-table-column>
    ...
    >
      ...
      <el-tabs v-model="activeName" style="margin-top: 15px">
        <el-tab-pane label="ПАСПОРТ ТОВАРУ" name="product">
          <keep-alive>
            <table>
              <tr>
        ...
        <el-tab-pane label="ЛОГІСТИЧНІ ПАРАМЕТРИ" name="props">
          <keep-alive>
            <table>
              <thead>
                <tr>
        ...
        <el-tab-pane label="МАРКУВАННЯ ТОВАРУ (СКЛАД ПРОДУКТУ)">
          <keep-alive>
            <table>
              <thead>
                <tr>
                  <th colspan="3">Назва товару Постачальника</th>
                </tr>
                <tr>
                  <th colspan="3">
                    Батончик із крем-пастою з арахісу зі змаженим арахісом та
...
import categories from "@/api/my/categories.json";
...
export default {
  ...
  data() {
    return {
      ...
      activeName: "product",
      active_scr:
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTqCEpshJwJ8hZJrjPtmH3EVfbHN327f9xS_g&usqp=CAU",
      tabMapOptions: [
      ...
      fileList: [
        {
          name: "товар1.jpeg",
          url: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTqCEpshJwJ8hZJrjPtmH3EVfbHN327f9xS_g&usqp=CAU",
        },
        {
          name: "товар2.jpeg",
          url: "https://domkotlov.by/images/product/000/000471/426-novyj-plyus-vajlant.jpg",
        },
        {
          name: "товар3.jpeg",
          url: "https://teplovoz.ua/image/cache/catalog/products/rinnai-kmf-3_3-380x380.jpg",
        },
        {
          name: "товар4.jpeg",
          url: "https://i2.wp.com/teplogalaxy.ru/wp-content/uploads/2017/07/Usovershenstvovaniya-novyh-kotlov-Vaillant.png",
        },
      ],
      loading: false,
      ...
      rules: {
        price: [
          { required: true, message: "не должно быть пустым", trigger: "blur" },
        ],
        timestamp: [
          {
            type: "date",
  ...
  methods: {
    ...
    async getBrands() {
      let id = await this.$route.meta.shop_id;
      this.listLoading = true;
      // this.brands = brands.results.filter(el => this.type == el.status)
    ...
    async getCategories() {
      let id = await this.$route.meta.shop_id;
      this.listLoading = true;
      this.categories = categories.results.filter(
        (el) => this.type == el.status
      );
    ...
    handleUpdate(row) {
      ...
      this.dialogFormVisible = true;
      (this.activeName = "product"), this.getImg();
    ...
    async onfileSelect(e) {
      ...
      uploadImg(this.selectedFile, 1).then((res) => {
        let data = res.data;
